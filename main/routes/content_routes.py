...
def content_index():
    ...
    if form.validate_on_submit():
        ...
        if not owner_obj:
            flash(f'Owner with email {owner_email} does not exist!', 'danger')
            return redirect(url_for('content'))

        # validation - if content already exists, reload page with error msg
        if Content.find_by_name(content_name):
            flash(f'Content with name {content_name} already exists!', 'danger')
            return redirect(url_for('content'))

        # new content is saved and page is reloaded
